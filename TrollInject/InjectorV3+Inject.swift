...
extension InjectorV3 {
    ...
    func copyAndSign(_ assetURLs: [URL]) throws -> [URL] {
        
        precondition(!assetURLs.isEmpty, "No assets to be injected.")
        precondition(!assetURLs.contains {
            $0.pathExtension.lowercased() != "dylib"
        }, "Only dylib is supported.")
        
        let preparedAssetURLs = try preprocessAssets(assetURLs)
        return try copyAndSignDylibs(preparedAssetURLs)
    }

    // MARK: - Private Methods
    
    fileprivate func copyAndSignDylibs(_ assetURLs: [URL]) throws -> [URL] {
        guard !assetURLs.isEmpty else {
            throw Error.generic("No assets to be copied.")
        }
        ...
        return try copyfiles(assetURLs)
